...
export class ProfileClient {
	...
	public async getProfilesByOwner(
		...
		for (const addr of lookupAddresses) {
			if (useCache && this.cachedAddresses.has(addr)) {
				...
				// address not seen before so we need to look it up
		...
		if (newLookupAddresses.size > 0) {
			...
			if (newObjectIds.size > 0) {
				...
				for (const profile of profileObjects.values()) {
					if (profile) {
						// nulls have already been added to the result and cache above
	...
	public async getProfilesById(
		...
		for (const objectId of lookupObjectIds) {
			if (useCache && this.cachedObjects.has(objectId)) {
				...
				// object not seen before so we need to look it up
	 ...
	 */
	public async hasProfile(lookupAddress: string, useCache?: boolean): Promise<boolean> {
	...
	protected async fetchProfileObjectIds(
		...
		const deserializeResults = (blockResults: SuiExecutionResult[]): LookupResults => {
	...
	protected async fetchProfileObjects(
		...
		const allProfiles: PolymediaProfile[] = [];
...
function suiObjectToProfile(resp: SuiObjectResponse): PolymediaProfile | null {
	...
	if (!owner) {
		throw new Error(
			"Missing object owner. Make sure to fetch the object with `showOwner: true`",
		);
	}
	const isOwnedObject =
		typeof owner === "object" && ("AddressOwner" in owner || "ObjectOwner" in owner);
	if (!isOwnedObject) {
